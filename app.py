...
st.set_page_config(page_title="MR League", layout="centered")
...
sheet_escalacoes = spreadsheet.worksheet("escalacoes_rodadas")
sheet_jogadores = spreadsheet.worksheet("dados_jogadores")

@st.cache_data(ttl=3600)
def carregar_dados_jogadores():
    data = sheet_jogadores.get_all_records()
    return pd.DataFrame(data)

@st.cache_data(ttl=3600)
def carregar_escalacoes():
    data = sheet_escalacoes.get_all_records()
    return pd.DataFrame(data)

df_jogadores = carregar_dados_jogadores()
jogadores_dict = dict(zip(df_jogadores["PLAYER"], df_jogadores["POSIÇÃO"]))
jogadores_disponiveis = sorted(df_jogadores["PLAYER"].tolist())

def get_jogadores_da_rodada(rodada, time):
    df = carregar_escalacoes()
    jogadores = df.query("RODADA == @rodada and TIME == @time")["JOGADOR"].tolist()
    return jogadores[:5]

# Layout
pagina = st.sidebar.selectbox("Escolha a página", ["🏃‍♂️ Escalar Times", "📋 Registrar Partida"])

# --------- TELA 1: ESCALAR TIMES ---------
if pagina == "🏃‍♂️ Escalar Times":
    st.markdown("## Escalar Times por Rodada")

    try:
        valores_rodadas = sheet_escalacoes.col_values(1)[1:]  # coluna "RODADA"
        valores_numericos = [int(v) for v in valores_rodadas if str(v).isdigit()]
        ultima_rodada = max(valores_numericos) if valores_numericos else 0
    except:
        ultima_rodada = 0

    rodada = st.number_input("Número da Rodada", min_value=1, value=ultima_rodada + 1, step=1)
    #rodada = st.number_input("Número da Rodada", min_value=1, step=1)
    time = st.selectbox("Selecione o Time", ["AMARELO", "BRANCO", "VERMELHO", "PRETO"])
    st.markdown("### Escolha os 5 Jogadores de Linha")

    jogadores_selecionados = []
    for i in range(5):
        jogador = st.selectbox(f"Jogador {i+1}", jogadores_disponiveis, key=f"jogador_{i}")
        jogadores_selecionados.append(jogador)

    if st.button("✅ Registrar Escalação"):
        for jogador in jogadores_selecionados:
            sheet_escalacoes.append_row([rodada, time, jogador])
        st.success("✅ Escalação registrada com sucesso!")

# --------- TELA 2: REGISTRAR PARTIDA ---------
elif pagina == "📋 Registrar Partida":
    st.markdown("""
    ...
    def render_time_inputs(time_nome, prefixo, jogadores_linha):
        ...
        jogador = st.selectbox("Goleiro", jogadores_disponiveis, key=f"{prefixo}_goleiro")
